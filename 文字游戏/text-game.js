var vm = new Vue({
	...
	methods:{
		...
		okUser(){
			...
			this.statement="角色创建成功，点击此处开始游戏";
			this.one = false;
			switch(this.job){
		...
		btnR(){
			switch(this.btnRight){
				...
				case "暗中观察":{this.statement="谨慎的你决定在观察村庄，毕竟防范之心不可无。（HP-2）";this.btnLeft = "进入村子";this.user.HP = this.user.HP-2;break};
				...
				case "坚决离开":{this.statement="你离开了这个村子，打算自己寻找个简单的过夜之处。-----一天过去了，今天请做出你的选择";this.btnLeft = "进入村庄";this.btnRight = "忍受饥饿";break};
		...
		shayezhu(){
			...
			switch(this.job){
				...
				case "射手":{this.statement = "未能一击毙命，凶猛的野猪冲了上来，拼着受伤杀死了野猪（HP-2）。获得野猪的皮（防御值+5）-----点击继续";this.two=false;this.two=false;this.user.防御 = this.user.防御+5;this.HP=this.user.HP-2;tt.onclick=this.textClick;break}
